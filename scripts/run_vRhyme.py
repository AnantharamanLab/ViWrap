...
def run_vrhyme(viral_scaffold, vRhyme_outdir, mapping_result_dir, threads):  
    ...
    cmd = f'vRhyme -i {viral_scaffold} -g {viral_scaffold_ffn} -p {viral_scaffold_faa} -c {mapping_result_dir}/vRhyme_input_coverage.txt -t {int(threads)} -o {vRhyme_outdir} --red 2 1> /dev/null'
    os.system(cmd)      
