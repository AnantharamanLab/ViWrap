...
def fetch_arguments(parser,root_dir,db_path_default):
    ...
    parser.add_argument('--out_dir','-o', dest='out_dir', required=True, default='./ViWrap_outdir', help=r'(required) output directory to deposit all results (default = ./ViWrap_outdir) output folder to deposit all results. ViWrap will exit if the folder already exists')
    parser.add_argument('--clean_wo_reads', dest='clean_wo_reads', action='store_true', required=False, default=False, help=r"If you used 'run_wo_reads' to generate results, you will need to add flag '--clean_wo_reads' to clean redundant information")
...
def set_defaults(args):
    ...
    args['vcontact2_outdir_wo_reads'] = os.path.join(args['out_dir'],'01_vContact2_outdir')
    args['iphop_outdir_wo_reads'] = os.path.join(args['out_dir'],'04_iPHoP_outdir')
    args['iphop_custom_outdir_wo_reads'] = os.path.join(args['out_dir'],'04_iPHoP_outdir/iPHoP_outdir_custom_MAGs')
...
def main(args):
    ...
    if not args['clean_wo_reads']:
        ...
        # Step 2 Clean 01_vContact2_outdir
        os.system(f"rm {os.path.join(args['vcontact2_outdir_wo_reads'], 'combined_viral_faa.*')}")
        os.system(f"rm {os.path.join(args['vcontact2_outdir_wo_reads'], 'modules.ntwk')}")
        os.system(f"rm {os.path.join(args['vcontact2_outdir_wo_reads'], 'combined_pro2viral_gn_map.csv')}")      
        
        time_current = f"[{str(datetime.now().replace(microsecond=0))}]"
        logger.info(f"{time_current} | 01_vContact2_outdir has been cleaned")
        

        # Step 3 Clean 04_iPHoP_outdir
        os.system(f"rm -r {os.path.join(args['iphop_outdir_wo_reads'], 'Wdir')}")
        if args['custom_MAGs_dir'] != 'none':
            os.system(f"rm -r {os.path.join(args['iphop_custom_outdir_wo_reads'], 'Wdir')}")
        
        time_current = f"[{str(datetime.now().replace(microsecond=0))}]"
        logger.info(f"{time_current} | 04_iPHoP_outdir has been cleaned")        
