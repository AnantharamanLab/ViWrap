...
def run_vibrant(metagenomic_scaffold, viwrap_outdir, threads, virome, input_length_limit, db_dir):
    ...
    if virome:
        cmd = f'VIBRANT_run.py -i {metagenomic_scaffold} -folder {viwrap_outdir} -t {int(threads)} -l {input_length_limit} -d {db_dir}/VIBRANT_db -m {db_dir}/VIBRANT_db/files 1> /dev/null'
    else:
        cmd = f'VIBRANT_run.py -i {metagenomic_scaffold} -folder {viwrap_outdir} -t {int(threads)} -virome -l {input_length_limit} -d {db_dir}/VIBRANT_db -m {db_dir}/VIBRANT_db/files 1> /dev/null'
    os.system(cmd)     
